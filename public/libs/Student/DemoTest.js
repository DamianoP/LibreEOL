...
function showSubjectInfoAndExams(selected) {
  ...
  $.ajax({
    url: "index.php?page=subject/showsubjectinfoandexamsdemotest",
    type: "post",
    data: {
      idSubject: JSON.parse($(subjectRowSelected).attr("value"))['subject'],
      file: JSON.parse($(subjectRowSelected).attr("value"))['file']
    },
