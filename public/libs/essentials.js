...
function newLightbox(panel, lightboxOptions){
    var defaultOptions = { closeClick      :   false,
                           closeEsc        :   false,
                           destroyOnClose  :   true,
                           zIndex          :   950,
        ...
        onLoad          :   function(){
                                setTimeout(function(){
                                    ...
                                    $("#modalSuccess").slideUp({
                                        complete : function(){ $("#modalSuccess").trigger("close"); }
                                    });
                                } , 1500);
                            }
    });
...
function closeLightbox(panel){
    panel.slideUp({
        complete : function(){ panel.trigger('close'); $("body").removeClass("noscroll");}
    });
}
...
var opened=false;
$(function(){
    $("ul.topnav li a.trigger").mouseenter(
        function(){
            if(opened){
    ...
    $("ul.topnav li a.trigger").click(
        function(){
            if(!opened){
                ...
                $(this).parent().hover(
                    function(){},
                    function(){$(this).parent().find("ul.subnav").slideUp('fast')
                    }
                )
            }else{
...
function printBoxHelpMessage(message){
    ...
    return '<div class="boxHelpMessage clearer">' +
           '    <div class="left"><img src="themes/default/images/help.png" /></div>' +
           '    <div class="left" style="'+style+'">'+message+'</div>' +
           '</div>';
}
...
function updateCharsCounter(element, max){
    ...
    if(chars < 0){
        $(element+"Chars").text(chars).addClass("overlimit");
    } else {
        $(element+"Chars").text(chars).removeClass("overlimit");
    }
}
...
$(function(){
    if($("#dialogError").length > 0){
        ...
        $("#dialogError").find(".ui-button-text").keypress(function(e) {
            if ((e.which && e.which == 13) || (e.keyCode && e.keyCode == 13) || // enter
                (e.which && e.which == 27) || (e.keyCode && e.keyCode == 27)    // esc
                ) {
...
function errorDialog (title, message) {
    ...
    $("#dialogError").dialog( "option", "title", title )
                     .dialog("open");
