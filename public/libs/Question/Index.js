...
//                { className: "qText", width : "650px" },
    ...
    $("#questionsTableContainer .ui-corner-bl").append(printBoxLogMessage(ttHelpLog));
...
function showTopicInfo(selectedTopic) {
    clearTimeout(timer);
    topicRowEdit = $(selectedTopic);
    $.ajax({
        url     : "index.php?page=subject/showtopicinfo",
        type    : "post",
        data    : {
            action  :   "show",
            idTopic :   $(selectedTopic).attr("value")
        },
        success : function (data) {
            if(data == "NACK"){
                //alert(data);
...
function changeQuestionStatus(selectedQuestionAndConfirm){
    ...
    var newStatus, newStatusText, imageName;
    switch($(rowTable.data()[qtci.status]).attr("value")){
        case "i" : newStatus = "a"; newStatusText = ttActive; imageName="Active"; break;
        case "a" : newStatus = "i"; newStatusText = ttInactive; imageName="Inactive"; break;
        case "e" : newStatus = "a"; newStatusText = ttError; imageName="Error"; break;
    }
...
//                alert(data);
                if(data == "ACK"){
                    ...
                    $(questionStatus).attr("value", newStatus)
                        ...
                        .attr("src", imageDir + imageName + ".png");
...
function helpjs(){
    $("#dialogError p").html(ttHelpQuestion);
