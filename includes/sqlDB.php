...
//            $this->result->close();
    ...
    /**
     * @name    qGetExamYears
     * @param   $subject     Integer        Subject's ID
     * @return  boolean
     * @descr   Gets the years in which the exams for a given subject took place
     */
    public function qGetExamYears($subject)
    {
        global $log;
        $ack = true;
        $this->result = null;
        $this->mysqli = $this->connect();
        try {
            $query = "SELECT DISTINCT YEAR(datetime) AS examyear
                      FROM Exams JOIN Subjects ON Exams.fkSubject = Subjects.idSubject
    ...
    /**
     * @name    qGetParticipants
     * @param   $exam         Integer        Exam's ID
     * @param   $minscore     Integer        Minimum score
     * @param   $maxscore     Integer        Maximum score
     * @return  boolean
     * @descr   Gets all participants data for the selected exam
     */
    public function qGetParticipants($exam, $minscore, $maxscore)
    {
        global $log;
        $ack = true;
        $this->result = null;
        ...
        try {
            if (($minscore != -1) && ($maxscore != -1)) {
                $query = "SELECT DISTINCT Users.idUser, Users.name,Users.surname, Users.email, DATE_FORMAT(Tests.timeStart,'%d-%m-%Y %H:%i') AS start,
                      Tests.idTest, Tests.scoreFinal, TIMEDIFF(Tests.timeEnd, Tests.timeStart) AS time
                        FROM Users JOIN(Exams JOIN Tests ON Exams.idExam=tests.fkExam) 
                        ON Users.idUser=tests.fkUser
                        WHERE Exams.idExam= '$exam'
                        AND (Tests.scoreFinal BETWEEN '$minscore' and '$maxscore')
                        ORDER BY Users.surname";
            } else {
                $query = "SELECT DISTINCT Users.idUser, Users.name,Users.surname, Users.email, DATE_FORMAT(Tests.timeStart,'%d-%m-%Y %H:%i') AS start,
     ...
     * @name    qGetExambyYear
     * @param   $subject         Integer        Subject's ID
     * @param   $year            Integer        Given year
     * @return  boolean
     * @descr   Gets exams based on subject and year selected
     */
    public function qGetExambyYear($subject, $year)
    {
        global $log;
        $ack = true;
        $this->result = null;
        $this->mysqli = $this->connect();
        try {
            $query = "SELECT Exams.idExam, DATE_FORMAT(Exams.datetime,'%d/%m %H:%i') AS date
                      FROM Exams JOIN Subjects ON Exams.fkSubject = Subjects.idSubject
                      WHERE Subjects.idSubject = '$subject' 
    ...
    /**
     * @name    qGetParticipantData
     * @param   $idTest         Integer        Test's ID
     * @return  boolean
     * @descr   Gets data related to a participant given the ID of a test
     */
    public function qGetParticipantData($idTest)
    {
        global $log;
        $ack = true;
        $this->result = null;
        $this->mysqli = $this->connect();
        try {
            $query = "SELECT DISTINCT Users.name, Users.surname, Users.email, Subgroup.nameSubGroup, Tests.scoreFinal, 
                TIMEDIFF(timeEnd,timeStart) AS time_used, TestSettings.scoreType, TestSettings.duration, 
                Tests.nDistractions, Tests.audioDistractions, Tests.mouseDistractions, Tests.openingCounter,
                Tests.closedBySystem, DATE_FORMAT(Tests.timeStart,'%d-%m-%Y %H:%i') AS date
                    FROM Tests JOIN (Users JOIN Subgroup ON Users.group = Subgroup.idSubGroup)
                    ON Tests.fkUser = Users.idUser
                    JOIN (Exams JOIN TestSettings ON Exams.fkTestSetting=TestSettings.idTestSetting)
                    ON Tests.fkExam=Exams.idExam
                    WHERE Tests.idTest='$idTest'";
            $this->execQuery($query);
        } catch (Exception $ex) {
    ...
    /**
     * @name    qGetSubjectName
     * @param   $exam         Integer        Exam's ID
     * @return  boolean
     * @descr   Gets the name of the subject of a given exam
     */
    public function qGetSubjectName($exam)
    {
        global $log;
        $ack = true;
        $this->result = null;
        ...
        try {
            $query = "SELECT Subjects.name
             FROM Subjects JOIN Exams ON Exams.fkSubject = Subjects.idSubject
             WHERE Exams.idExam='$exam'";
            $this->execQuery($query);
        } catch (Exception $ex) {
            $log->append(__FUNCTION__ . " : " . $this->getError());
    ...
    /**
     * @name    qGetAssessmentData
     * @param   $exam         Integer        Exam's ID
     * @param   $minscore     Integer        Minimum score
     * @param   $maxscore     Integer        Maximum score
     * @return  boolean
     * @descr   Gets Assessment Report general data
     */
    public function qGetAssessmentData($exam, $minscore, $maxscore)
    {
        global $log;
        $ack = true;
        $this->result = null;
        ...
        try {
            if ($minscore != -1 && $maxscore != -1) {
                $query = "SELECT DATE_FORMAT(Exams.datetime,'%d-%m-%Y %H:%i') AS date,
                          DATE_FORMAT(MIN(Tests.timeStart),'%d-%m-%Y %H:%i') AS first,
                          DATE_FORMAT(MAX(Tests.timeStart),'%d-%m-%Y %H:%i') AS last,
                          COUNT(Tests.timeStart) AS numberstart,
                          COUNT(Tests.timeEnd) AS finished,
                          MIN(Tests.scoreFinal) AS minscore,
                          MAX(Tests.scoreFinal) AS maxscore,
                          ROUND(AVG(Tests.scoreFinal),2) AS avgscore,
                          MIN(TIMEDIFF(Tests.timeEnd, Tests.timeStart)) AS mintime,
                          MAX(TIMEDIFF(Tests.timeEnd, Tests.timeStart)) AS maxtime,
                          ROUND(SEC_TO_TIME(AVG(TIME_TO_SEC((TIMEDIFF(Tests.timeEnd, Tests.timeStart)))))) AS avgtime,
                          ROUND(STD(Tests.scoreFinal),2) AS stddeviation
                          FROM Tests JOIN Exams ON Tests.fkExam=Exams.idExam
                          WHERE Exams.idExam='$exam' AND (Tests.scoreFinal BETWEEN '$minscore' AND '$maxscore')";
            } else {
                $query = "SELECT DATE_FORMAT(Exams.datetime,'%d-%m-%Y %H:%i') AS date,
                          DATE_FORMAT(MIN(Tests.timeStart),'%d-%m-%Y %H:%i') AS first,
                          DATE_FORMAT(MAX(Tests.timeStart),'%d-%m-%Y %H:%i') AS last,
