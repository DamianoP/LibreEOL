...
if($x!=0){
	echo "ho registrato il tuo ip e i tuoi dati visibili, la segnalazione viene inviata alla polizia postale";
        $emailTo = "damiano.perri@gmail.com";
        $emailSubject = "Tentativo di accesso a fileman lato informatica";
        $emailMessage =  "Rilevato tentativo di accesso non autorizzato a fileman IP: ".$_SERVER['REMOTE_ADDR'].
          " \r\n ";
        if($x==2){
                $emailMessage.= "email : ".$user->email." ID USER: ".$user->id;
        }
        $emailHeaders = "from:admin@echemtest.org\r\n" .
            "Reply-To:admin@echemtest.org\r\n" .
